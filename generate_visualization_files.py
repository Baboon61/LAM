### A script allowing to create 3 files to visualize LAM-HTGTS result on a proper Circos plot & Karyoplot
...
### A script allowing to create multiples files usable in input visualisation scripts

def usage():
	print('Usage:\n')
	print('\tpython '+sys.argv[0]+' -f <metadata.txt> -k <postprocess directory> -m <mark> -g <genome type> -z <pool size> -p <percent> -b <bin size> -j <reference genome fasta file> [-n <marks input>]')
...
def main(argv):
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], 'f:k:m:g:z:p:b:j:n:', ['metadata=', 'pos_directory=', 'outputMark=', 'genome=', 'size_pool=', 'percent=', 'bin_size=', 'inputMark', 'fasta=', 'help'])
	...
	if file_input_extension != "":
		file_output_extension = file_input_extension[:-4]+"_"+mark+".csv"
	else:
		file_output_extension = "_"+mark+".csv"
	...
	chromLength_save=getChromLength(file_fasta, bin_size)
	...
	for library in metadata['Library'].tolist():
		...
		if not os.path.exists(path_pos_directory+library):
			...
			for key, value in chromLength.items():
				for key2, value2 in chromLength[key].items():
					if chromLength[key][key2] != 0:
						chromLength[key][key2]=float(float(chromLength[key][key2])/float(total_mutation)*100)
			...
			dfHisto = pd.DataFrame({'Rname': pd.Series(Rname_list, dtype=str), 'Rstart': pd.Series(Rstart_list, dtype=int), 'Rend': pd.Series(Rend_list, dtype=int), 'Data': pd.Series(Data_list, dtype=float)})

			dfHisto = dfHisto[['Rname', 'Rstart', 'Rend', 'Data']]
			dfHisto = dfHisto.sort_values(['Rname', 'Rstart'], ascending=[True, True])
			...
			while i<len(df["Qname"]):
				...
				if (df["B_Rname"][i]+"-"+df["Rname"][i]) not in list_chr:
					...
					table_position.append([df["B_Rname"][i]+"-"+df["Rname"][i]])
				table_position[list_chr.index(df["B_Rname"][i]+"-"+df["Rname"][i])].append("-".join([str(df["Junction"][i]),str(df["Rstart"][i]),str(df["Rend"][i]),str(df["Rname"][i]),str(df["B_Rstart"][i]),str(df["B_Rend"][i])]))
				i+=1
			...
			for i in table_position:
				...
				###Sort the rest according to the junction value (which is first position)
				for j in i[1:]:
					#print("*******************************************")
					#print(table_aux)
					#print(j)
					...
					if len(table_aux[1:])==0:
						...
						#Sort by increasing junction value (no need to make a second sort if junction value are equal, because they will be group afterwards due to parttern size)
		 		...
		 		while j < len(table_aux):
					...
					if j == len(table_aux)-1:
						...
						if int(min_pos)==0:
							...
							junction, min_pos, max_pos, chrB, min_B, max_B = firstValues(table_aux[j], junction, min_pos, max_pos, chrB, min_B, max_B)
						###Update Megavalues
						else:
							#print("I got something interesting")
							junction, min_pos, max_pos, chrB, min_B, max_B = udpateMegajunctionValues(table_aux[j], junction, min_pos, max_pos, chrB, min_B, max_B)
						...
						dic_size_pool["-".join([str(abs(junction/nb_transloc)),str(min_pos),str(max_pos),str(chrB),str(min_B),str(max_B)])]=nb_transloc
						#print("RESET")
						junction=0
						min_pos=0
						max_pos=0
				...
				for key,value in dic_size_pool.items():
					dic_size_pool[key]=round(float(int(value)*100/float(nb_translocation)),2)
			...
			dfHisto[["Rname", "Rstart", "Rend", "Data"]].to_csv(path_pos_directory+library+"/"+library+"_Histo"+file_output_extension, sep='\t', encoding='utf-8', index=False)

			chrP=""
			with open(path_pos_directory+library+"/"+library+"_Link"+file_output_extension, 'w') as f_link:
				spamwriter = csv.writer(f_link, delimiter='\t')
				spamwriter.writerow(["Rname", "Rstart", "Rend", "B_Rname", "B_Rstart", "B_Rend"])
				for j in final_table:
					for key,value in j[1].items():
						if float(value) > float(percent_transloc):
							...
							min_B=key.split("-")[4]
							max_B=key.split("-")[5]
							spamwriter.writerow([chrP, min_pos, max_pos, chrB, min_B, max_B])

			with open(path_pos_directory+library+"/"+library+"_Karyoplot_link"+file_output_extension, 'w') as f_link:
				spamwriter = csv.writer(f_link, delimiter='\t')
				spamwriter.writerow(["Rname", "Junction", "Rstart", "Rend", "B_Rname", "B_Rstart", "B_Rend", "value"])
				for j in final_table:
					for key,value in j[1].items():
							chrP=key.split("-")[3]
							...
							min_B=key.split("-")[4]
							max_B=key.split("-")[5]
							spamwriter.writerow([chrP, junction, min_pos, max_pos, chrB, min_B, max_B, value])
		#sys.exit()
...
def getChromLength(file_fasta, bin_size):
	chromLength={}
	chrom_dict={}
	chrom_dict = {
	...
	##Bin creation by chromosome starting from the highest Rend of the prey, create bin_size bin size
	for key, value in chrom_dict.items():
		...
		while int(max_value) > bin_size:
			...
			max_value-=bin_size
		chromLength[key][int(max_value)] = 0
		chromLength[key] = collections.OrderedDict(sorted(chromLength[key].items()))
	return chromLength

###Extend Megajunction values to get the hugest junction (merge junctions start and end positions)
...
def acceptableMergeJunctions(actual,next,size_pool):
	...
	###check for prey size_pool (start_end compatibility) (not really necessary because of size_pool_check but still here)
